...
def fastAllArgwhere(ar):
    ...
    _, indicesEnd = np.unique(ar1[-1::-1],
                              return_index=True)  # This is probably needless and can be found from indicesStart
...
def haplotypePlotter(predCNA, doCluster=False, withLinkage=[], saveFile='', chr=[], plotSize=[], vertLine=[]):
    ...
    if doCluster:
        if len(withLinkage) == 0:
            if len(chr) != 0:
                g = sns.clustermap(predCNA, col_cluster=False, row_cluster=True, linewidths=0.0, cmap=cmap,
                                   cbar_pos=None, yticklabels=False, xticklabels=False, col_colors=chr_colors)
            else:
                g = sns.clustermap(predCNA, col_cluster=False, row_cluster=True, linewidths=0.0, cmap=cmap,
                                   cbar_pos=None, yticklabels=False, xticklabels=False)
        else:
            if len(chr) != 0:
                g = sns.clustermap(predCNA, col_cluster=False, row_cluster=True, row_linkage=withLinkage[0],
                                   linewidths=0.0, cmap=cmap, cbar_pos=None, yticklabels=False, xticklabels=False,
                                   col_colors=chr_colors)
            else:
                g = sns.clustermap(predCNA, col_cluster=False, row_cluster=True, row_linkage=withLinkage[0],
                                   linewidths=0.0, cmap=cmap, cbar_pos=None, yticklabels=False, xticklabels=False)
