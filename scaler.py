...
CNA = np.sum(CNA, axis=2)


RDR = loadnpz(RDR_file)
HAP = loadnpz(BAF_file)
bins = loadnpz(bins_file)
argBin = np.argwhere(bins == 13)[:, 0]

CNA = CNA[:, bins]
CNAmean = np.mean(CNA, axis=1)


RDR = RDR * CNAmean.reshape((-1, 1))


HAP_total = np.sum(HAP, axis=0)
weight_total = np.sum(HAP_total, axis=1)
BAF_total = HAP_total[:, 1] / np.sum(HAP_total, axis=1)

RDR_total = np.mean(RDR, axis=0)
...
weight_total = weight_total[argBin]
weight_total = 0.3 * weight_total / np.mean(weight_total)

print (HAP_total[argBin])

var = multiHapNoise(HAP1)
...
start2 = argBin[0] - 20
end2 = argBin[-1] + 1 + 20
argBin2 = np.arange(end2 - start2) + start2


#sns.clustermap(  RDR[:, argBin2]  , col_cluster=False, row_cluster=True,linewidths=0.0)
#plt.show()
#quit()

plt.plot( argBin2, BAF_total[argBin2] )
plt.plot( argBin, BAF_total[argBin]  )
plt.plot( argBin, weight_total )
plt.plot( argBin, np.zeros(argBin.shape[0]) + 0.666, c='grey' )
plt.show()

plt.plot( argBin2, RDR_total[argBin2] / 3 )
plt.plot( argBin, RDR_total[argBin] / 3 )
plt.plot( argBin, weight_total )
plt.show()
